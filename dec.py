...
#print df.head()
df.to_csv("/decSales.csv", index = False)
...
for i in range(len(results)):
    ...
    if len(a)==3:
        ...
        quantities = df.loc[(df['ItemID']== itemNoList[len(itemNoList)-1]) & ((df['Hours']) == apriori_byname[2]) & (df['Segment'] == apriori_byname[1]) , 'Quantity'].sum()
...
combinationFromApriori.to_csv("/media/satyam/DiskG/3-1/Data Mining/DM Assignment/Dataset/Decdetail.csv", index = False)
decRevenue = combinationFromApriori['Revenue'].sum()
